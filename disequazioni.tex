...
{\centering\captionof{table}{Regola del DICE}\index{Disequazione!secondo grado!segno trinomio}
		\begin{tikzpicture}
	...
	\end{tikzpicture}\par}\index{Delta}\index{Discriminante}
\section{Segno del trinomio delta uguale a zero}%{\textDelta uguale a zero}
{\centering\captionof{table}{Regola Segue a}
		\begin{tikzpicture}
	\tkzTabInit[color,lgt=5,espcl=3]%
	{$x$ / .8, $\Delta=0$\\ Il segno di\\ $ax^2+bx+c$ / 2}%
	{$-\infty$,$x_1$,$+\infty$}%
...
{\centering\captionof{table}{Regola  Solo a}
		\begin{tikzpicture}
...
{\centering\captionof{table}{Equazione e disequazione di secondo grado}
	\begin{tabular}{CCCC}
\toprule
...
\bottomrule
 \end{tabular}\par}\index{Delta}\index{Discriminante}\index{Disequazione!secondo grado!soluzione}
\begin{figure}[ht]
	\centering
